...
/**
 * Based off of Mutation Pipeline
 *
...
public class MapElitesMutationPipeline extends MapElitesBreedingPipeline
{
                       ...
                       final int thread)
    {
        ...
        int n = sources[0].produce(min,max,start,inds,state,thread);

        // should we bother?
        if (!state.random[thread].nextBoolean(likelihood))
            return reproduce(n, start, inds, state, thread, false);  // DON'T produce children from source -- we already did
        ...
        for(int q=start; q < n+start; q++)
        {
            ...
            for(int x=0;x<numTries;x++)
            {
                ...
                p1 = nodeselect.pickNode(state, 0,thread,i,i.trees[t]);
            ...
            if (sources[0] instanceof MapElitesBreedingPipeline)
