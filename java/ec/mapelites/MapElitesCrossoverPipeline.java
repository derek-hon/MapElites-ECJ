...
/**
 * Based off of Koza's CrossoverPipeline
 */
...
public class MapElitesCrossoverPipeline extends MapElitesBreedingPipeline
{
    ...
    public Elite parents[];

    public MapElitesCrossoverPipeline() { parents = new Elite[2]; }
    ...
    public boolean verifyPoints(final MapElitesInitializer initializer,
...

    {
        ...
        MapElitesInitializer initializer = (MapElitesInitializer)state.initializer;

        for(int q=start;q<n+start; /* no increment */)  // keep on going until we're filled up
        {
            // grab two individuals from our sources
            if (sources[0]==sources[1])  // grab from the same source
                sources[0].produce(2,2,0,parents,state,thread);
            else // grab from different sources
            {
                sources[0].produce(1,1,0,parents,state,thread);
                sources[1].produce(1,1,1,parents,state,thread);
            }
            ...
            if (tree1!=TREE_UNFIXED && (tree1<0 || tree1 >= parents[0].trees.length))
                // uh oh
                state.output.fatal("MapElites Crossover Pipeline attempted to fix tree.0 to a value which was out of bounds of the array of the individual's trees.  Check the pipeline's fixed tree values -- they may be negative or greater than the number of trees in an individual");
            if (tree2!=TREE_UNFIXED && (tree2<0 || tree2 >= parents[1].trees.length))
                // uh oh
                state.output.fatal("MapElites Crossover Pipeline attempted to fix tree.1 to a value which was out of bounds of the array of the individual's trees.  Check the pipeline's fixed tree values -- they may be negative or greater than the number of trees in an individual");
            ...
            else
            {
                ...
                if (parents[0].trees[t1].constraints(initializer)
                    ...
                    state.output.fatal("MapElites Crossover Pipeline's two tree choices are both specified by the user -- but their GPTreeConstraints are not the same");
            }
            ...
            Elite j1 = (Elite)(parents[0].lightClone());
            Elite j2 = null;
            if (n-(q-start)>=2 && !tossSecondParent) j2 = (Elite)(parents[1].lightClone());
