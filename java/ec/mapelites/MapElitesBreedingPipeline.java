...
public abstract class MapElitesBreedingPipeline extends MapElitesBreedingSource implements MapElitesSourceForm
{
    ...
    /** Standard parameter for individual-selectors associated with a MapElitesBreedingPipeline */
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        for(int x=0;x<sources.length;x++) {
            ...
            if (s!=null && s.equals(V_SAME)) {
                ...
                sources[x] = (MapElitesBreedingSource)
                        (state.parameters.getInstanceForParameter(
                                p,d,MapElitesBreedingSource.class));
                         ...
                         boolean produceChildrenFromSource)
    {
        if (produceChildrenFromSource)
            sources[0].produce(n,n,start,inds,state,thread);
        if ((MapElitesBreedingSource)sources[0] instanceof MapElitesSelectionMethod)
    ...
    public void prepareToProduce(final EvolutionState state,
        ...
        for(int x=0;x<sources.length;x++)
            if (x==0 || sources[x]!=sources[x-1])
                sources[x].prepareToProduce(state,thread);
    }
    ...
    public void finishProducing(final EvolutionState state,
        ...
        for(int x=0;x<sources.length;x++)
            if (x==0 || sources[x]!=sources[x-1])
                sources[x].finishProducing(state, thread);
    }
    ...
    public void individualReplaced(final MapElitesEvolutionState state,
        ...
        for(int x=0; x<sources.length;x++)
            ((MapElitesSourceForm)(sources[x])).individualReplaced(state,thread,individual);
    }
